...
namespace FileParty.Core.Tests
{
    public class FilePartyRegistrationShould
    {
        ...
        [Fact]
        async Task ThrowIfChangingAnUnregisteredStorageProvider()
        {
            var sc = this.AddFileParty(cfg =>
            {
                cfg.AddModule<TestModule2>(new TestConfiguration2());
        ...
        public async Task GetSpecializedStorageServices(Type factory, Type serviceInterface, bool? useConfig)
        {
            ...
            switch (factoryService)
            {
                case IFilePartyFactory f:
                {
                    var storageProvider = 
                        useConfig == null ? (
                            serviceInterface == typeof(IStorageReader) ? (object) f.GetStorageReader() :
                            serviceInterface == typeof(IStorageWriter) ? f.GetStorageWriter() : null
                        ) : 
                        useConfig.GetValueOrDefault(true) ? (
                            serviceInterface == typeof(IStorageReader) ? (object) f.GetStorageReader(new TestConfiguration()) :
                            serviceInterface == typeof(IStorageWriter) ? f.GetStorageWriter(new TestConfiguration()) : null
                        ) : (
                ...
                case IAsyncFilePartyFactory af:
                {
                    var storageProvider = 
                        useConfig == null ? (
                            serviceInterface == typeof(IAsyncStorageReader) ? (object) await af.GetAsyncStorageReader() :
                            serviceInterface == typeof(IAsyncStorageWriter) ? await af.GetAsyncStorageWriter() : null
                        ) : 
                        useConfig.GetValueOrDefault(true) ? (
                            serviceInterface == typeof(IAsyncStorageReader) ? (object) await af.GetAsyncStorageReader<TestModule>(new TestConfiguration()) :
                            serviceInterface == typeof(IAsyncStorageWriter) ? await af.GetAsyncStorageWriter<TestModule>(new TestConfiguration()) : null
                        ) : (
                            serviceInterface == typeof(IAsyncStorageReader) ? (object) await af.GetAsyncStorageReader<TestModule>() :
                            serviceInterface == typeof(IAsyncStorageWriter) ? await af.GetAsyncStorageWriter<TestModule>() : null
                        );
