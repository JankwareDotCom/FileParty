...
public class StoredItemInformationShould
{
    ...
    public void TryGetPropertySuccessfully()
    {
        ...
        Assert.True(info.TryGetProperty("string", out var str, "failed"));
        Assert.Equal("string", str);

        Assert.True(info.TryGetProperty("bool", out var b, false));
        Assert.True(b);

        Assert.True(info.TryGetProperty<object>("null", out var obj, new object()));
        Assert.Null(obj);

        Assert.True(info.TryGetProperty("datetime", out var dtprop, DateTime.UtcNow));
        Assert.Equal(dt, dtprop);

        Assert.True(info.TryGetProperty("integer", out var integer, 8675309));
        Assert.Equal(42, integer);

        Assert.True(info.TryGetProperty("float", out var f, 3.14159f));
        Assert.Equal(69.420f, f);

        Assert.False(info.TryGetProperty("doesnotexist", out var dne, "does not exist"));
        Assert.Equal("does not exist", dne);

        Assert.False(info.TryGetProperty(null, out var propNameEmpty, "prop name empty"));
